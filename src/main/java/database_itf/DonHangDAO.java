...
public class DonHangDAO implements InterfaceDAO<DonHang> {
	...
	public int insert(DonHang t) {
		int kq = 0;
		Connection con = JDBCUtil.getConnection();
		String sql = "INSERT INTO donhang(madonhang, makhachhang, diachiamuahang, diachinhanhang, trangthai, hinhthucthanhtoan, trangthaithanhtoan ,sotiendathanhtoan, sotienconthieu, ngaydathang, ngaygiaohang)"
				+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		try {
			PreparedStatement st = con.prepareStatement(sql);
			st.setString(1, t.getMaDonHang());
	...
	public int update(DonHang t) {
		...
		String sql = "UPDATE donhang" + " SET " + "khachhang=?" + ", diachinguoimua=?" + ",diachinguoinhan=?"
				+ ",trangthai=?" +",hinhthucthanhtoan=?" + ",trangthaithanhtoan=?" + ",tienthanhtoan=?" + ",tienthieu=?" + ",ngaydathang=?"
